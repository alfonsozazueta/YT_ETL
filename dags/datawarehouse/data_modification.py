...
def insert_rows(cur, conn, schema, row):
    ...
    try:
        if schema == 'staging':
            video_id = 'video_id'
            cur.execute(
                f"""INSERT INTO {schema}.{table}("Vide_ID", "Video_title", "Upload_Date","Duration","Video_Views","Likes_Count","Comments_Count")
...
def update_rows(cur, conn, schema, row):
    ...
    try:
        ...
        cur.execute(
            ...
            WHERE "Video_ID" = %({video_title})s AND "Upload_Date" = %({upload_date})s;
                """, row
        )
...
def delete_rows(cur,conn,schema, ids_to_delete):
    ...
    try:
        ...
        logger.error(f'Deleted rows with Video_IDs: {ids_to_delete} - {e}')
        raise e
